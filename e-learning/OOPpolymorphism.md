...
なぜポリモーフィズムがあるかというと，どのサブクラスであったとしても，Client のコードはそのままで実行できるという利点があるからです。この利点により，コードの再利用性が高まるので，ソフトウェア開発の生産性が高くなります。このことをサンプルプログラムを見て実感してください。
...
ペット型ロボットのコードが動作する時に，各行をどういう順番で実行するのかを説明してください。とくにメソッドの呼び出しがあった際に，どのクラスの値が変数に代入されているために，ポリモーフィズムの働きで，どのメソッドが呼ばれることになるのかを詳述してください。
...
ただし，次のような価格を取得できるようにしてください。(すなわち，スーパークラスの GetPrice をオーバーライドします)
...
演習問題4で考えた，ポリモーフィズムになりそうな例をグループで共有しましょう。例に対しお互いに感想を述べ合いましょう。もし改善した方がいい点があれば，指摘をしましょう。

### 演習問題6

演習問題5で共有した，ポリモーフィズムになりそうな例を実装しましょう。
